...
let initialSpeed = 1000
  ...
  //*ENEMY ENERGIZE VULNERABILITY
  function addCharacterEnergized(enemyCurrentPosition) {
      cells[enemyCurrentPosition].classList.add('character-energized')
  }
...
function energize() {
  if (cells[characterCurrentPosition].classList.contains('energizer')) {
    ...
    addEnergizerEnemyOne(enemyOneCurrentPosition)
    addEnergizerEnemyTwo(enemyTwoCurrentPosition)
    addEnergizerEnemyThree(enemyThreeCurrentPosition)
    addEnergizerEnemyFour(enemyFourCurrentPosition)
    score += 90
  }
  currentScore.innerText = score
...
function addEnergizerEnemyOne(position) {
...
function addEnergizerEnemyTwo(position) {
  cells[position].classList.add(energizedClass)
}
...
function addEnergizerEnemyThree(position) {
  cells[position].classList.add(energizedClass)
}
...
function addEnergizerEnemyFour(position) {
  cells[position].classList.add(energizedClass)
}
...
function locateEnemyOne() {
  ...
  if (cells[characterCurrentPosition].classList.contains('energizer')) {
    cells[enemyOneCurrentPosition].classList.add('character-energized')
  }
}
...
function locateEnemyTwo() {
  if (cells[characterCurrentPosition].classList.contains('energizer')) {
    addCharacterEnergized(enemyTwoCurrentPosition)
    }
  const route = Math.floor(Math.random() * 12)
...
function locateEnemyThree() {
  if (cells[characterCurrentPosition].classList.contains('energizer')) {
    addCharacterEnergized(enemyThreeCurrentPosition)
    }
  const route = Math.floor(Math.random() * 12)
...
function locateEnemyFour() {
  if (cells[characterCurrentPosition].classList.contains('energizer')) {
    addCharacterEnergized(enemyFourCurrentPosition)
    }
  const route = Math.floor(Math.random() * 12)
  ...
  if (currentDate.getMinutes() % 2 === 0) {
    ...
    //* TOP RIGHT
      ...
      if (((inTrapCharacterTopRightGoDown(enemyFourCurrentPosition)) && (characterRow(characterCurrentPosition) !== enemyRow(enemyFourCurrentPosition))) && spaceAboveDoesNotContainEnemy) {
        ...
        if (route < 6 && spaceBelowDoesNotContainEnemy) {
          enemyFourCurrentPosition += width
        } else if (route >= 6 && spaceLefttDoesNotContainEnemy) {
          enemyFourCurrentPosition -= 1
        }
      } else if (vacantBottom(enemyFourCurrentPosition) && spaceBelowDoesNotContainEnemy) {
        enemyFourCurrentPosition += width
